...
import { setPlayerAction, setComputerAction, setRoundResult, updatedUses, setSelectedSpecial } from '../actions/index.js';
...
class ActionList extends Component {
  ...
  handleActionClick = (playerAction) => {
    ...
    if (this.state.visionActive) {
      this.timeout = setTimeout(() => {
        this.computeRoundResult(playerAction, this.props.computerAction)
      }, 1000);
    } else {
      ...
      this.timeout = setTimeout(() => {
        this.computeRoundResult(playerAction, computerAction)
      }, 1000);
    }
  ...
  computeRoundResult = (playerAction, computerAction) => {
    let roundResult = null;

    if (playerAction === computerAction) {
      roundResult = "Both sides suffer damages";
    } else if (playerAction === "Melee Attack" && computerAction === "Ranged Attack") {
      roundResult = "You deal damage to your foe but suffer from his range attack";
    } else if (playerAction === "Ranged Attack" && computerAction === "Magic Attack")  {
      roundResult = "You deal damage to your foe";
    } else if (playerAction === "Magic Attack" && computerAction === "Melee Attack") {
      roundResult = "You deal heavy damage to your foe"
    } else if (computerAction === "Melee Attack" && playerAction === "Ranged Attack") {
      roundResult = "The foe deals damage to you but suffer from your range attack";
    } else if (computerAction === "Ranged Attack" && playerAction === "Magic Attack") {
      roundResult = "The foe deals damage to you";
    } else if (computerAction === "Magic Attack" && playerAction === "Melee Attack") {
      roundResult = "The foe deals heavy damage to you";
    }

    this.props.setRoundResult(roundResult);
  }
